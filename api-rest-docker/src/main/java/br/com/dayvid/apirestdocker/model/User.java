...
public class User implements UserDetails, Serializable {
    ...
    @ManyToMany(fetch = FetchType.EAGER) //Many to many significa muitos para muitos ou seja ambas as tabalas podem ter mais de uma informação da outra ou seja um usuario pode ter muitas permissões e uma permissão pode ter varios usuarios
    @JoinTable(name = "user_permission", joinColumns = {@JoinColumn (name = "id_user")},
        inverseJoinColumns = {@JoinColumn (name = "id_permission")}
    )
    private List<Permission> permissions; //se implementa a tabela intermediaria na tabela mais forte pois tem apenas 2 foreing keys caso fosse algo maior deveria ter uma entidade propria
